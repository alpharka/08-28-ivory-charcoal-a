...
const gallery = [
  { src: "https://images.unsplash.com/photo-1519741497674-611481863552?auto=format&fit=crop&w=1200&q=85", alt: "Pasangan berjalan berdampingan di bawah cahaya sore", caption: "01 — langkah pertama", category: "Pre-wedding" },
  { src: "https://images.unsplash.com/photo-1511285560929-80b456fea0bc?auto=format&fit=crop&w=1200&q=85", alt: "Detail tangan pasangan dengan suasana hangat", caption: "02 — ruang yang sama", category: "Engagement" },
  { src: "https://images.unsplash.com/photo-1537633552985-df8429e8048b?auto=format&fit=crop&w=1200&q=85", alt: "Pasangan tersenyum di suasana alam terbuka", caption: "03 — hari yang ringan", category: "Pre-wedding" },
  { src: "https://images.unsplash.com/photo-1519225421980-715cb0215aed?auto=format&fit=crop&w=1200&q=85", alt: "Momen intim pasangan dengan latar dedaunan", caption: "04 — dekat", category: "Behind the Scenes" },
  { src: "https://images.unsplash.com/photo-1460364157752-926555421a7e?auto=format&fit=crop&w=1200&q=85", alt: "Bunga pernikahan bernuansa putih dan hangat", caption: "05 — detail kecil", category: "Behind the Scenes" },
  { src: "https://images.unsplash.com/photo-1519167758481-83f550bb49b3?auto=format&fit=crop&w=1200&q=85", alt: "Ruang acara dengan dekorasi bunga minimal", caption: "06 — tempat pulang", category: "Engagement" },
];
...
export default function Home() {
  ...
  const filteredGallery = useMemo(() => activeCategory === "Semua" ? gallery : gallery.filter((item) => item.category === activeCategory), [activeCategory]);
  ...
  useEffect(() => { const onKey = (event: KeyboardEvent) => { if (lightbox === null) return; if (event.key === "Escape") setLightbox(null); if (event.key === "ArrowRight") setLightbox((lightbox + 1) % filteredGallery.length); if (event.key === "ArrowLeft") setLightbox((lightbox - 1 + filteredGallery.length) % filteredGallery.length); }; window.addEventListener("keydown", onKey); return () => window.removeEventListener("keydown", onKey); }, [lightbox, filteredGallery.length]);
  ...
  return <div className={`invitation ${opened ? "is-open" : ""}`}>
    ...
    <main id="top">
      ...
      <section id="gallery" className="gallery-section section-pad"><SectionIntro number="03" eyebrow="Enam fragmen kecil" title="Yang ingin kami ingat" /><div className="gallery-filters" role="group" aria-label="Filter kategori galeri">{["Semua", "Pre-wedding", "Engagement", "Behind the Scenes"].map((category) => <button key={category} className={activeCategory === category ? "filter-button is-active" : "filter-button"} onClick={() => { setActiveCategory(category); setLightbox(null); }} aria-pressed={activeCategory === category}>{category}</button>)}</div><div className="gallery-grid">{filteredGallery.map((item, index) => <button className={`gallery-item item-${(index % 6) + 1}`} key={item.src} onClick={() => setLightbox(index)} aria-label={`Lihat foto ${index + 1}: ${item.category}`} data-reveal><img src={item.src} alt={item.alt} /><span>{item.caption} · {item.category}</span></button>)}</div><p className="gallery-count">Menampilkan {filteredGallery.length} dari {gallery.length} fragmen</p></section>
    ...
    {lightbox !== null && <div className="lightbox" role="dialog" aria-modal="true" aria-label="Galeri foto" onClick={() => setLightbox(null)}><button className="lightbox-close" onClick={() => setLightbox(null)} aria-label="Tutup"><X /></button><button className="lightbox-prev" onClick={(e) => { e.stopPropagation(); setLightbox((lightbox - 1 + filteredGallery.length) % filteredGallery.length); }} aria-label="Foto sebelumnya"><ArrowLeft /></button><figure onClick={(e) => e.stopPropagation()}><img src={filteredGallery[lightbox].src} alt={filteredGallery[lightbox].alt} /><figcaption>{filteredGallery[lightbox].caption} · {filteredGallery[lightbox].category}</figcaption></figure><button className="lightbox-next" onClick={(e) => { e.stopPropagation(); setLightbox((lightbox + 1) % filteredGallery.length); }} aria-label="Foto berikutnya"><ArrowRight /></button></div>}
  </div>;
}
